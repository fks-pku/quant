...
- QMT live trade-history recovery is a read-only post-close job: it may connect to the real broker and import missing fills, but it must not run `--simulate-daily`, `--pending-only`, or broker order submission. Post-close live pending generation should not continue after recovery fails, and this path must not enable a 09:30 real-order task.
