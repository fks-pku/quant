...
- PaperBroker simulated fills must use the shared `quant.runtime.execution_commission` model used by backtests; cash, avg_cost, fill callbacks, recorder rows, and dashboard NAV must all include the same commission total.
...
- Paper mode is a local backtest-style route: initialize only `PaperBroker`, load execution-date bars, fill against execution-date open, compute commission through the shared backtest/paper execution cost model, and send local fills through the same FillHandler/recorder path.
...
- Strategy dashboard state is now fully migrated into `strategy_state.duckdb` strict tables. Legacy daily JSONL and `strategy_modes/<mode>/<strategy_name>/` JSONL may be read as migration/raw inputs, but dashboard status, records, holdings, metrics, pending orders, curves, operations, runs, watermarks, and reconciliations must read from the strict state store. All strategy facts must carry `run_id` when an active run exists.
