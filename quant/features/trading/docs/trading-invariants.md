...
T8-02  BUY record_order(symbol, order_value) writes pending order value only to the owning strategy RiskEngine; SELL record_order() counts order rate only and must not reserve BUY cash.
...
T8-07  Strategy-mode PaperBroker execution must resolve the owning SubPortfolio before calling the shared execution simulator; master portfolio holdings/cash must not satisfy or reject a managed strategy's SELL/T+1/cash gates.
T8-08  Broker-side rejected paper orders must be recorded as rejected strategy_orders with the broker order id and rejection reason, not as submitted/no-fill attempts.
```

### 对应测试: `test_order_manager_uses_strategy_specific_risk_engine`, `test_order_manager_does_not_reserve_cash_for_sell_orders`, `test_order_manager_submit_existing_signal_records_broker_rejection`, `test_paper_broker_uses_strategy_portfolio_for_simulated_sell`, `test_fill_handler_updates_strategy_specific_portfolio`, `test_fill_handler_buy_deducts_strategy_shadow_cash`, `test_quant_system_dispatches_fill_only_to_owning_strategy`, `test_context_exposes_strategy_scoped_order_surface_only` in `test_live_trading_records.py`
