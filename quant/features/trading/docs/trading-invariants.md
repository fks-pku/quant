...
T13-16  Dashboard payloads must expose per-mode operations, strict run/control state, recovery status, watermarks, and top-level operations health from durable `strategy_state.duckdb` records
...
T13-22  Each strategy_name and mode must maintain strict state rows in `strategy_state.duckdb` for operations, runs, control state, capital events, signals, submit attempts, orders, fills, positions, NAV snapshots, watermarks, and reconciliations. Legacy daily JSONL and `strategy_modes/<mode>/<strategy_name>/` JSONL are migration/raw inputs only; dashboard status, records, holdings, metrics, pending orders, and curves must be derived from the strict state store, never by mixing another strategy or mode.
