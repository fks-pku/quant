...
T9-13  PaperBroker simulated fills must compute commission through the same shared runtime model used by backtests; paper cash, avg_cost, fill callbacks, recorder rows, dashboard order rows, and NAV must include that commission instead of hard-coding zero.
```

### 对应测试: `test_live_execution_manager_*`, `test_trading_context_*`, `test_paper_broker_uses_execution_open_for_limit_fill_and_callbacks`, `test_order_manager_flushes_paper_broker_fills_after_submission`, `test_paper_broker_commission_matches_backtest_execution_for_cn_etf`, `test_quant_system_rejects_external_broker_adapter_in_paper_mode` in `test_live_trading_records.py`; `test_qmt_limit_price_rounds_to_exchange_tick_preserving_side_bound`, `test_submit_order_normalizes_limit_price_before_qmt_call`, `test_qmt_simulate_trade_mode_refuses_order_submission` in `test_qmt_broker.py`
...
2. Runtime startup restores tracked strategy positions and SubPortfolio lots before any strategy hook runs, so signals are generated from the same holdings the dashboard displays.
...
4. On the next trading day, order execution is the only stage allowed to consume due pending signals. Live execution is guarded by the local QMT morning submit path with open-price polling and duplicate locks; paper execution waits for execution-date bars and fills locally through PaperBroker at the execution-day open/limit contract.
...
6. Post-execution facts flow back into the same DuckDB state store: submitted orders, fills, commissions, positions, NAV snapshots, watermarks, recovery results, and operations ledgers stay scoped by `strategy_name` and mode.
...
T13-08  Dashboard order rows are filled only by the server-side display contract: open_price is the order-date daily open, paper fill_price equals limit_price, live fill_price equals broker fill price, commission is reported separately, historical paper fills with zero recorded commission are estimated through the shared runtime backtest/paper commission model, order/fill matching uses broker/client order id plus record_date so daily PaperBroker id reuse cannot cross-match fills, and slippage_bps satisfies fill_price = open_price * (1 + slippage_bps / 10000)
