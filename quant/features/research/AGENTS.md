...
- If the active checklist passes, the strategy belongs under `quant/features/strategies/<strategy_id>/` and the same `full_research_report.html` must be copied into that strategy directory. If the checklist fails or lacks required evidence, keep/archive the strategy under `quant/features/strategies/reject/<strategy_id>/`. Parameter sensitivity can be run incrementally later and is not a promotion blocker unless the user explicitly raises it into the active checklist.
