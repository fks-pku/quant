...
- Walk-forward audit reports must show each Go / No-Go threshold next to the observed value and per-metric verdict, including worst OOS Sharpe, profitable split ratio, DSR warning threshold, and capacity participation threshold.
- Final Go / No-Go must evaluate `production_gate.py` when both strict backtest and walk-forward evidence are present. The gate must record thresholds, observed metrics, failures, and warnings under `final_decision.scores.production_gate`; a strategy cannot show a final pass when the production gate fails.
...
- Archived strategies that opt into a point-in-time universe must let the archived PIT category maps and resolved universe override stale persisted fixed symbol lists for strict backtest, walk-forward, and report rendering.
