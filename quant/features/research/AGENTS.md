...
- `config/research.yaml` discovery `sources` currently route through arXiv, SSRN, NBER, blog RSS, A-share public forum seeds, and local A-share structural seeds.
...
- Discovery/scout upserts must not downgrade already researched idea-bank statuses such as `validated`, `validation_failed`, `candidate`, `rejected`, `stage1_rejected`, `needs_manual_spec`, or `error` back to `discovered`, `skipped`, or `research_queue`.
...
- Each end-to-end research run must write `full_research_report.html` plus the three independent Chinese stage reports: `fast_research_report.html`, `strict_backtest_report.html`, and `walkforward_audit_report.html`, each with a timestamped run copy. The full report must use exactly these top-level expandable cards: `1. Final Decision`, `2. 策略逻辑`, `3. 策略表现`, `4. 重要 Metric`, `5. Walk-forward`, `6. Stability`, and `7. Risk`. The `Final Decision` section is the active Go / No-Go checklist plus the executive snapshot; do not create a separate top-level `Metric Checklist` section. The strategy-logic section must include plain-language `策略白话逻辑`, `参数`, and `止盈止损逻辑`; all strategy-logic prose, rule summaries, parameter explanations, and risk-exit explanations in this section must be written in Chinese. Code field names, ticker symbols, and metric abbreviations may remain literal, but English prose must not be used as the explanation. The performance section must include the equity curve, yearly/monthly return calendar, and core performance table. The important-metric section must include cost, data quality, turnover and exposure, and capacity. The default full-report entrypoint must run and persist both Walk-forward and Stability audit payloads before the final HTML refresh; explicit single-stage or quick-debug commands may opt out. The walk-forward section must render purged OOS methodology, summary, and split evidence, and missing payloads must display `not_run`/missing explicitly rather than numeric zero split counts. The stability section must render parameter-sensitivity / one-factor perturbation evidence when present, or an explicit not-run audit table when absent; stability evidence must not select new production parameters. Default reports use `risk_exit.enabled=true`; disabled risk-exit runs are parameter-sensitivity/ablation artifacts and must not be rendered as default report comparisons. The active checklist is evaluated from `max_adv_participation <= 5% ADV`, `total_trades > 50`, and the CAGR/MaxDD tier gate (`5%-10% CAGR => MaxDD <=15%`, `10%-15% => <=25%`, `15%-20% => <=30%`, `20%+ => <=50%`). The full report must not render an `Appendix` or `TODO` section unless the user explicitly requests an incremental audit artifact. Walk-forward evidence is retained in the independent walk-forward stage report and the full-report Walk-forward card but is not part of the current checklist. Parameter sensitivity is an optional incremental artifact in strict stage reports, but the full report must expose the Stability card. Missing required checklist evidence is a fail, and strategy-type-specific non-applicable evidence must be rendered as `n/a` with a reason in the relevant stage report. Full reports must render execution-cost BPS in the cost summary: weighted and median effective BPS from `execution_cost_bps.weighted_effective_bps` and `execution_cost_bps.median_effective_bps`, where effective BPS is `slippage_bps + impact_bps` from execution observations and excludes explicit commission/tax. User-facing reports must not emit raw `not_recorded`. Do not generate or maintain `full_research_report.md`. Any report format change must update `quant/tests/test_research_report_contract.py` in the same change.
...
- Full-report important metrics must render `Barra-like Factor Decomposition` when `factor_decomposition` evidence is present. This is A-share style-factor return attribution, not licensed MSCI Barra data. The self-built CN style set is `MKT`, `SIZE`, `VALUE`, `MOM`, `REV`, `VOL`, and `LIQ`; missing evidence must render as `not_run` rather than silently disappearing.
...
- A-share strict research backtests must attach an enabled execution cost model for every CN strategy: small-cap/low-price/cap-field strategies use `small_cap_realistic`, ETF/LOF/fund rotation uses `cn_etf_liquidity_impact`, and other A-share daily strategies use `cn_daily_liquidity_impact`. The backtest/full-report cost contract is: D close uses only D-or-earlier data to freeze `+x bps`; D+1 submission derives the executable LIMIT from open/reference (`BUY = reference*(1+x/10000)`, `SELL = reference*(1-x/10000)`).
...
- Walk-forward strict audit must reuse the same strict execution-cost model family, archived strategy parameters, and configured CN initial cash (`default_initial_cash`, currently 10000) as standalone strict backtests; it must not silently fall back to fixed 5bps-only fills or stale capital.
...
| Low-frequency public scout CLI | `quant/scripts/run_low_frequency_idea_scout.py`, `config/research.yaml` |
