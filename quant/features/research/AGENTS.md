...
- Each end-to-end research run must write `full_research_report.html` plus the three independent Chinese stage reports: `fast_research_report.html`, `strict_backtest_report.html`, and `walkforward_audit_report.html`, each with a timestamped run copy. The full report must use an executive-summary-first layout: final decision, current Go / No-Go checklist, strategy logic summary, core performance, cost/capacity summary, key risks, then folded audit appendices for fast research, strict diagnostics, walk-forward, and artifact links. The active checklist is `max_adv_participation <= 5% ADV`, `total_trades > 50`, and the CAGR/MaxDD tier gate (`5%-10% CAGR => MaxDD <=15%`, `10%-15% => <=25%`, `15%-20% => <=30%`, `20%+ => <=50%`). Walk-forward evidence is retained as audit evidence but is not part of the current checklist. Missing required checklist evidence is a fail, and strategy-type-specific non-applicable evidence must be rendered as `n/a` with a reason. User-facing reports must not emit raw `not_recorded`. Do not generate or maintain `full_research_report.md`. Any report format change must update `quant/tests/test_research_report_contract.py` in the same change.
...
- Walk-forward strict audit must reuse the same strict execution-cost model family, archived strategy parameters, and CN initial-cash sizing convention as standalone strict backtests; it must not silently fall back to fixed 5bps-only fills.
...
- Archived strategies that opt into a point-in-time universe must let the archived PIT category maps and resolved universe override stale persisted fixed symbol lists for strict backtest, walk-forward, and report rendering. ETF category strategies should use `dynamic_pit_category_wide` by default: the configured symbol superset may cover the full backtest window, but each rebalance may select only candidates with bar, PIT fund-size, liquidity, and lookback evidence visible by that date; holdings are then selected by the strategy signal from that visible wide universe. `universe_as_of` and `universe_max_symbols_per_category=1` are robustness/stress modes, not the default production evidence. ETF reports must include an ETF metadata survivorship audit when fund metadata lacks delist markers or bar-only symbols exist.
...
- Formal A-share research defaults to strict backtests from `2016-01-01` through `2025-12-31` and reports must include a yearly strategy-return calendar from structured strict Backtester equity results.
