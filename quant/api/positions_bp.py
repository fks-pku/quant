...
def get_portfolio():
    return jsonify(state.portfolio_data)
...
def create_order():
    ...
    price = state.MOCK_PRICES.get(symbol, round(random.uniform(50, 500), 2))
    status = 'FILLED'

    order = {
        'id': len(state.orders_data) + 1,
        'symbol': symbol,
        'quantity': quantity,
        'side': side,
        'price': price,
    ...
    state.orders_data.append(order)
...
def get_orders():
    return jsonify(state.orders_data[-20:])
...
def get_market_data():
    ...
    for sym in ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'VIX']:
        price = state.MOCK_PRICES.get(sym, 100)
...
def strategy_history(name):
    return jsonify({"strategy": name, "snapshots": state._get_strategy_history(name)})


@positions_bp.route('/api/strategy/all-history', methods=['GET'])
def all_strategy_history():
    return jsonify(state._get_strategy_history())
