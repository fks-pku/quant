...
- `estimate_cost_protection_limit(...)` — 纯函数：用 D 日可知 reference/bar 和 `execution_cost_model` 估算成本保护 LIMIT，不读未来 bar、不提交订单
...
- 日线 MARKET 目标单生成成本保护 LIMIT 时，策略传入 price 或 D 日 signal bar close 优先；resolver 只作为缺失信号参考价时的兜底，不能用 D+1 quote 放宽 D 日限价

## 修改守则

- 改 lifecycle 分发语义时，必须同步更新 backtest/trading 相关测试
- 改 daily snapshot 语义时，必须覆盖完整快照、缺失 symbol、实盘事件流触发时点
- 改 execution reference 语义时，必须覆盖 D 日信号价优先、signal bar close 兜底、resolver 兜底、缺 quote 不下单
- 改 execution cost 估算时，必须同步覆盖 backtest/live/paper 的 D 日限价一致性测试
