"""Shared execution commission model for backtest and paper trading."""

import datetime as dt
import math
from typing import Any, Dict, Optional

from quant.domain.models.market import detect_market
...
CN_EXCHANGE_TRADED_FUND_PREFIXES = ("15", "16", "18", "50", "51", "52", "56", "58")


def get_rate_for_date(rate_history: Dict[dt.date, float], trade_date: Optional[dt.date], default: float) -> float:
    if not rate_history:
        return default
    if trade_date is None:
        return rate_history[max(rate_history)]
    applicable = [start for start in rate_history if start <= trade_date]
    if not applicable:
        return default
    return rate_history[max(applicable)]
...
def calculate_commission(
    ...
    trade_value = abs(float(price) * float(quantity))
    market = detect_market(str(symbol).split(".")[0])
...
def total_commission(
    ...
    return sum(calculate_commission(symbol, price, quantity, side, commission_config, trade_date).values())
...
def _calculate_us_commission(quantity: float, trade_value: float, side: str, commission_config: Any) -> Dict[str, float]:
    ...
    elif cfg.get("type") == "percent":
        commission = max(trade_value * cfg.get("percent", 0.0), cfg.get("min_per_order", 0.0))
    else:
        commission = 0.0
    result = {"commission": commission}
    if side == "SELL":
        result["sec_fee"] = trade_value * US_SEC_FEE_RATE
        result["finra_taf"] = min(quantity * US_FINRA_TAF_PER_SHARE, 8.30)
    return result
...
def _calculate_hk_commission(
    ...
    stamp_rate = get_rate_for_date(HK_STAMP_DUTY_RATE_HISTORY, trade_date, HK_STAMP_DUTY_RATE)
    stamp_duty = math.ceil(trade_value * stamp_rate)
