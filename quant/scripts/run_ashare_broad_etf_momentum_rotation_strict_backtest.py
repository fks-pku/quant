"""Run strict backtests for broad ETF risk-adjusted momentum rotation."""
...
STRATEGY_ID = "ashare_broad_etf_momentum_rotation"
TITLE = "宽基 ETF 风险调整动量轮动"
...
SCENARIOS: List[Dict[str, Any]] = [
    {
        "name": "core_risk_adjusted_momentum_70pct",
        "symbols": DEFAULT_BROAD_ETF_SYMBOLS,
    ...
    {
        "name": "core_risk_adjusted_momentum_75pct",
        "symbols": DEFAULT_BROAD_ETF_SYMBOLS,
        "momentum_lookback": 180,
        "trend_window": 120,
        "volatility_window": 60,
    ...
    {
        "name": "core_risk_adjusted_momentum_80pct",
        "symbols": DEFAULT_BROAD_ETF_SYMBOLS,
        "momentum_lookback": 180,
        "trend_window": 120,
        "volatility_window": 60,
    ...
    {
        "name": "core_risk_adjusted_momentum_85pct",
        "symbols": DEFAULT_BROAD_ETF_SYMBOLS,
        "momentum_lookback": 180,
        "trend_window": 120,
        "volatility_window": 60,
    ...
    {
        "name": "core_risk_adjusted_momentum_90pct",
        "symbols": DEFAULT_BROAD_ETF_SYMBOLS,
        "momentum_lookback": 180,
        "trend_window": 120,
        "volatility_window": 60,
    ...
    {
        "name": "core_risk_adjusted_momentum_93pct",
        "symbols": DEFAULT_BROAD_ETF_SYMBOLS,
        "momentum_lookback": 180,
        "trend_window": 120,
        "volatility_window": 60,
    ...
    {
        "name": "lower_turnover_252d_momentum_90pct",
        "symbols": DEFAULT_BROAD_ETF_SYMBOLS,
...
<tr><td>1. 更新数据</td><td>读取当日宽基 ETF 日线，信号只使用当日收盘及历史数据，订单下一交易日开盘执行。</td><td>避免使用未来价格，严格保留 T+1 执行时滞。</td></tr>
<tr><td>2. 候选过滤</td><td>ETF 需要有足够 180 日历史、20 日平均成交额达标、180 日动量为正，且收盘价高于 120 日均线。</td><td>只在中长期趋势向上且流动性可交易时承担风险。</td></tr>
<tr><td>3. 信号打分</td><td>score = 180 日收益率 / 60 日年化波动率。</td><td>同等动量下优先选择波动更低、趋势更平滑的宽基 ETF。</td></tr>
<tr><td>4. 调仓</td><td>每 60 个交易日选择得分最高的 1-2 只 ETF；风险总敞口为场景参数，其余资金保持真实现金。</td><td>避免频繁追涨杀跌，也不依赖货币 ETF 价格序列的分红处理。</td></tr>
<tr><td>5. 日度风控</td><td>持仓 ETF 若跌破趋势或动量转负，即使未到调仓日也先卖出并等待下一次轮动。</td><td>把退出规则和轮动买入分开，降低深回撤中被 holding window 卡住的风险。</td></tr>
</tbody></table></div>
"""
...
def _run_one(
    ...
    execution_cost_model = _strict_execution_cost_model(
        ...
        {
            "name": TITLE,
            "description": "CN-listed broad ETF risk-adjusted momentum rotation",
