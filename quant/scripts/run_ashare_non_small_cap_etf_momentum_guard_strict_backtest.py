"""Run strict backtests for non-small-cap ETF momentum guard rotation."""
...
SCENARIOS: List[Dict[str, Any]] = [
    {
        "name": "weekly_126d_mom_vol20_top3_98pct",
        "symbols": DEFAULT_NON_SMALL_CAP_ETFS,
    ...
    {
        "name": "weekly_126d_mom_vol20_top3_95pct",
        "symbols": DEFAULT_NON_SMALL_CAP_ETFS,
        "momentum_lookback": 126,
        "momentum_skip": 1,
        "trend_window": 120,
    ...
    {
        "name": "weekly_126d_mom_vol60_top3_98pct",
        "symbols": DEFAULT_NON_SMALL_CAP_ETFS,
        "momentum_lookback": 126,
        "momentum_skip": 1,
        "trend_window": 120,
    ...
    {
        "name": "weekly_126d_mom_vol20_top2_98pct",
        "symbols": DEFAULT_NON_SMALL_CAP_ETFS,
        "momentum_lookback": 126,
        "momentum_skip": 1,
        "trend_window": 120,
    ...
    {
        "name": "weekly_126d_any_positive_mom_top3_98pct",
        "symbols": DEFAULT_NON_SMALL_CAP_ETFS,
        "momentum_lookback": 126,
        "momentum_skip": 1,
        "trend_window": 120,
...
<tr><td>1. 更新数据</td><td>读取默认 ETF 池的日线，信号只使用当日收盘及历史复权价，订单下一交易日开盘执行。</td><td>严格区分信号价格和成交价格，避免未来函数。</td></tr>
<tr><td>2. 非小市值 universe</td><td>默认池包含上证50、沪深300、创业板/创业板50、红利、券商/证券、半导体、黄金、港股和海外 ETF；排除中证500、中证1000等小盘 proxy。</td><td>用资产类别和流动性定义候选池，而不是事后挑单一赢家。</td></tr>
<tr><td>3. 候选过滤</td><td>候选必须有足够历史、20 日平均成交额达标、126 日动量超过阈值，并且收盘价高于 120 日均线。</td><td>只有趋势和流动性同时可接受时才承担风险。</td></tr>
<tr><td>4. 信号打分</td><td>score = 126 日收益率 / 20 或 60 日年化波动率；每 5 个交易日选择得分最高的 2-3 只 ETF。</td><td>同等涨幅下偏好波动更低、趋势更平滑的资产。</td></tr>
<tr><td>5. 组合与执行</td><td>按候选数量等权配置到目标风险敞口；没有合格候选时卖出 ETF 并持有真实现金。</td><td>不使用货币 ETF 平滑净值，也不写死单票持仓。</td></tr>
<tr><td>6. 严格约束</td><td>Backtester 执行 T+1、ETF 基金佣金、100 份手数、停牌/涨跌停约束和 cn_etf_liquidity_impact。</td><td>报告目标为年化收益超过 10%，最大回撤控制在 30% 以内。</td></tr>
</tbody></table></div>
"""
...
def _run_one(
    ...
    execution_cost_model = _strict_execution_cost_model(
        ...
        {
            "name": TITLE,
            "description": "CN-listed non-small-cap ETF momentum guard rotation",
        ...
        "metrics": {
            ...
            "research_stage_conclusions": {
                "strict_backtest": {
                    ...
                    "method": "项目 Backtester；信号收盘生成、订单 T+1 开盘执行；ETF 基金佣金、手数约束、涨跌停/停牌约束、流动性冲击成本。",
                }
            },
        },
        "evidence": {
