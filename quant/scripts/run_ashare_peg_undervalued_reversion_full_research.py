"""Run full research report for the A-share PEG undervalued reversion candidate."""
...
from quant.features.strategies.reject.ashare_peg_undervalued_reversion.strategy import (  # noqa: E402
...
STRATEGY_ID = "ashare_peg_undervalued_reversion"
TITLE = "A股 PEG 低估回归"
...
STRATEGY_PARAMS: Dict[str, Any] = {
    ...
    "max_growth_for_peg": 120.0,
    "min_roe": 5.0,
    "min_sales_growth": -20.0,
    "min_momentum": -0.25,
    "max_entry_peg": 0.60,
    "fair_peg_exit": 1.05,
    ...
    "risk_exit": {
        "enabled": True,
        "stop_loss_pct": 0.22,
        "min_stop_loss_pct": 0.18,
        "max_stop_loss_pct": 0.32,
        "stop_volatility_multiplier": 4.0,
        "take_profit_pct": 0.60,
        "trailing_stop_pct": 0.20,
        "trailing_volatility_multiplier": 3.0,
        "max_trailing_stop_pct": 0.35,
        "hard_take_profit_pct": 0.0,
        "exit_volatility_lookback": 20,
        "max_holding_days": 252,
        "min_time_stop_return": 0.0,
    },
}
...
SOURCE_URL = "user_prompt"


DETAIL_SECTION = """
<h3>策略执行流程</h3>
<div class="table-wrap"><table><thead><tr><th>步骤</th><th>执行逻辑</th><th>信息边界</th></tr></thead><tbody>
<tr><td>1. 股票池</td><td>排除普通账户受限板块 300/301/688/689，再过滤 ST、停牌、不可交易、退市状态、低价和低成交额股票。</td><td>使用当日 status、OHLCV、daily_basic 和账户权限约束。</td></tr>
<tr><td>2. PEG 计算</td><td>PEG = pe_ttm / growth_pct；growth_pct 优先使用 q_netprofit_yoy，缺失时退回 netprofit_yoy，且增长率用于 PEG 时封顶 120%。</td><td>财务字段按 ann_date 做 point-in-time backward asof join，不使用未来财报。</td></tr>
<tr><td>3. 低估筛选</td><td>入场要求 0 &lt; PEG &lt;= 0.60，PE_TTM 3-80，利润增速至少 8%，ROE 至少 5%，收入增速不低于 -20%。</td><td>估值来自当日 daily_basic，质量和成长来自已披露财务指标。</td></tr>
<tr><td>4. 长持有排序</td><td>按 PEG 折价、inverse PEG、ROE、利润增速、盈利收益率、126 日动量和收入增速横截面排名。</td><td>只在当日可交易候选池内做 percentile rank。</td></tr>
<tr><td>5. 出场和风控</td><td>默认 60 个交易日调仓；PEG 回到 1.05 或增长破 8% 触发 thesis exit；另有 18%-32% 宽止损、60% 盈利后 20%-35% 跟踪止盈和 252 日时间止损。</td><td>信号收盘生成，订单 T+1 开盘执行；严格回测包含涨跌停、停牌、手数、佣金、滑点和 2% ADV 冲击。</td></tr>
</tbody></table></div>
"""
...
def _run_backtest(
    ...
    strategy = ASharePegUndervaluedReversionStrategy(symbols=symbols, **strategy_params)
...
def _run_fast_validation(
    ...
    horizon = int(strategy_params.get("holding_days", 60) or 60)
    step = horizon
    max_lookback = int(strategy_params.get("momentum_lookback", 126) or 126) + int(strategy_params.get("momentum_skip", 5) or 5) + 2
...
def _score_current_universe(strategy: ASharePegUndervaluedReversionStrategy) -> Dict[str, float]:
    raw_candidates = []
...
def _summarize_fast_validation(
    ...
    tests = ["monthly_rank_ic", "top_bottom_spread", "pit_financial_ann_date_join", "peg_threshold_user_design"]
    if not records:
        return {
...
def _hypothesis_row(
    ...
    return {
        "strategy_id": STRATEGY_ID,
        "title": TITLE,
        "thesis": "PEG 接近 1 代表估值与增长大体匹配；PEG 低于 0.6 的股票若增长和 ROE 没有破坏，可能存在估值均值回归空间。",
        "status": status,
        "stage": "full_research",
        "source": "user_defined_hypothesis",
        "source_url": SOURCE_URL,
        "decision_reason": _decision_reason(metrics, strict_report, walkforward),
        "metrics": metrics,
        "evidence": {
            "source": "User-defined PEG undervaluation thesis",
            "source_url": SOURCE_URL,
            "local_strategy": True,
            "discovery_quality": {
                "score": 0.64,
                "source_type": "user_hypothesis",
                "matched_terms": ["PEG", "低估", "估值回归", "长持有", "止盈止损"],
                "risk_flags": ["fundamental_data_lag_required", "peg_growth_base_effect", "value_trap_risk"],
            },
            "strategy_spec": {
                "strategy_id": STRATEGY_ID,
                "strategy_type": "a_share_cross_sectional_peg_value_reversion",
                "signal_formula_key": STRATEGY_ID,
                "prediction_direction": "higher_is_better",
                "symbols_count": len(symbols),
                "universe": symbols,
                "universe_source": "daily_cn_ochl A-share symbols observed in the backtest window, excluding board prefixes 300/301/688/689 for ordinary-account tradability",
                "lookback_days": int(strategy_params.get("momentum_lookback", 126)),
                "horizon_days": int(strategy_params.get("holding_days", 60)),
                "execution_lag_days": 1,
                "rebalance_frequency": f"every {int(strategy_params.get('holding_days', 60))} trading days",
                "required_fields": ASharePegUndervaluedReversionStrategy(symbols=[]).required_fields,
                "parameters": strategy_params,
...
def _stage_conclusions(
    ...
    return {
        "fast_research": {
            ...
            "conclusion": f"60 日横截面验证：Rank IC={rank_ic:.4f}，FDR p={fdr:.4f}，Top-Bottom Spread={float(metrics.get('long_short_spread') or 0.0):.2%}。",
            "method": "按 60 日持有期抽样，使用 PIT 财务字段计算 PEG 信号，再计算未来 60 日后复权收益的 Spearman Rank IC。",
        },
        "strict_backtest": {
            "label": "严格回测",
...
def _parameter_sensitivity(strict_report: Dict[str, Any], strategy_params: Dict[str, Any]) -> Dict[str, Any]:
    metrics = strict_report.get("metrics") or {}
    return {
        "status": "single_user_defined_parameter_set",
        "method": "User-defined PEG thresholds were frozen before strict backtest; no optimization grid was used in this run.",
        ...
        "rows": [
            {
                "name": "base_peg_060_long_holding_risk_exit",
                "parameters": strategy_params,
                "cagr": metrics.get("cagr"),
                "max_drawdown_pct": metrics.get("max_drawdown_pct"),
                "sharpe": metrics.get("sharpe"),
...
def _strategy_logic(strategy_params: Dict[str, Any]) -> Dict[str, Any]:
    risk_exit = strategy_params.get("risk_exit") or {}
    return {
        "core_idea": "PEG 低于 0.6 的股票可能处于盈利增长被低估阶段；若增长、ROE 和基本趋势未破坏，则长持有等待 PEG 向 1.0 附近回归。",
        "universe": "全 A 股日线股票池；排除 300/301/688/689 后，每个调仓日再过滤 ST、停牌、不可交易、非上市、低价、低流动性，并限制在当日 total_mv 15%-95% 分位。",
        "entry_filters": [
            f"PEG = pe_ttm / q_netprofit_yoy_or_netprofit_yoy <= {strategy_params['max_entry_peg']}",
            ...
            f"q_sales_yoy/or_yoy >= {strategy_params['min_sales_growth']}%",
            f"126d skipped momentum >= {strategy_params['min_momentum']:.0%}",
        ],
        "ranking_rule": "score = 35% PEG discount + 15% inverse PEG + 15% ROE + 15% profit growth + 10% earnings yield + 5% skipped momentum + 5% sales growth。",
        ...
        "exit_rule": f"PEG 升至 {strategy_params['fair_peg_exit']} 或利润增速跌破 {strategy_params['min_profit_growth']}% 时提前卖出；否则在下次调仓跌出 Top 组合时卖出。",
        "risk_exit": (
            f"enabled={bool(risk_exit.get('enabled', strategy_params.get('enable_risk_exit')))}; "
            f"stop_loss={risk_exit.get('min_stop_loss_pct'):.0%}-{risk_exit.get('max_stop_loss_pct'):.0%}; "
            f"trailing_take_profit starts after {risk_exit.get('take_profit_pct'):.0%} gain; "
            f"time_stop={risk_exit.get('max_holding_days')} trading days if return < {risk_exit.get('min_time_stop_return'):.0%}."
        ),
...
def _parameter_explanations() -> Dict[str, str]:
    return {
        "holding_days": "调仓和预期持有周期；60 个交易日近似季度，匹配估值修复等待期。",
        "max_positions": "最多持有股票数；受 20,000 初始资金和 A 股 100 股手数约束，默认不做过度分散。",
        "cap_percentile_low": "按当日 point-in-time total_mv 排名后的下分位过滤，避免微盘和壳风险主导。",
        "cap_percentile_high": "按当日 point-in-time total_mv 排名后的上分位过滤，避免只买巨型权重。",
        "max_entry_peg": "核心低估阈值；用户定义 PEG <= 0.6 为低估。",
        "fair_peg_exit": "估值回归退出阈值；PEG 接近 1.0 后不再强行等待更高估值。",
        "max_growth_for_peg": "计算 PEG 时的增长率封顶，降低低基数高增长造成的虚假低 PEG。",
        "min_profit_growth": "利润增长确认门槛；用已披露 q_netprofit_yoy/netprofit_yoy。",
        "min_roe": "质量门槛；避免只买低质量高增长。",
        "min_sales_growth": "收入增长底线；降低利润单项改善但主营恶化的风险。",
        "momentum_lookback": "价值陷阱保护窗口。",
        "momentum_skip": "跳过最近几天以降低短期反转噪声。",
        "min_momentum": "趋势确认底线；宽松设置以保留低估均值回归机会。",
        "min_turnover": "最低平均成交额门槛；由共享护栏计算。",
        "risk_exit": "默认启用的宽止盈止损包，关闭版本只适合专项消融研究。",
    }
