...
def _export_partitioned(
    ...
    for year, month in partitions:
        partition_dir = dataset_dir / f"year={int(year):04d}" / f"month={int(month):02d}"
        partition_dir.mkdir(parents=True, exist_ok=True)
        target = partition_dir / "data.parquet"
        if target.exists():
            target.unlink()
        partition_where = _partition_where_clause(dataset.date_column, where, int(year), int(month))
        conn.execute(
            f"""
            COPY (
                SELECT *
...
def _partition_where_clause(date_column: str, where: str, year: int, month: int) -> str:
...
def upload_lake(local_root: Path, remote_prefix: str, manifest: LakeManifest, dry_run: bool) -> None:
    ...
    run_rclone(["copyto", str(local_root / MANIFEST_NAME), f"{remote_prefix.rstrip('/')}/{MANIFEST_NAME}"], dry_run)
...
def partition_dirs(dataset_dir: Path) -> list[Path]:
    return sorted(path for path in dataset_dir.glob("year=*/month=*") if path.is_dir())
