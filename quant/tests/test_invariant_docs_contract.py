...
def test_trading_invariants_describe_strategy_dashboard_lifecycle():
    ...
    expected_fragments = [
        ...
        "Paper D+1 replay must consume due pending `strategy_signals` rows",
        "skip strategy warmup and daily snapshot hooks",
        "fall back to hook replay only when there is no due pending signal",
        "rejected, and failed order rows are terminal submit-window facts",
        "does not keep retrying a known risk/broker rejection",
        "must not reserve BUY cash",
        "Strategy-mode PaperBroker execution must resolve the owning SubPortfolio",
        "Broker-side rejected paper orders must be recorded as rejected strategy_orders",
        "the adapter rejection reason",
