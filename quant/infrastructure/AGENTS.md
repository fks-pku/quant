...
- `PaperBroker` is the only paper-trading execution adapter; it consumes local execution-date and previous bars, delegates deterministic matching to `quant.runtime.execution_simulator` (including marketable LIMIT fill-at-limit semantics), computes commission through `quant.runtime.execution_commission`, resolves strategy-owned portfolios only through an injected resolver, records adapter rejection reasons, and emits local trade callbacks for the shared fill recorder without importing `features/`.
